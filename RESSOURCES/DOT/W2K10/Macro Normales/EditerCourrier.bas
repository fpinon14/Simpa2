Attribute VB_Name = "EditerCourrier"
Option Explicit
Public Sub Main()

' Variables portant sur Fichier COURRIER.INI.
Dim sFicCourrierIni As String, sRepGenDoc, sCleEdition, sFicSauve, sTypeSauve, sMiseSousPli, sEdition, sFicSpool As String
Dim iNbrDoc As Integer
Dim DocPrincipal As Document

' Variables portant sur Fichier de paramêtres de l'application.
' [DNTMAIL1-2] on ajoute le repertoire des paragraphe suivant le canal, et le répertoire de stockage de courrier.dot
Dim sRepPara, sRepModele As String
...
' Dbt [MAP2UNC] AJOUT LBP
Dim sSuffixe As String
Dim sEntete As String, sNomDoc As String
Dim bRet As Boolean
Dim tsTabFichierTemporaire() As String
' Fin [MAP2UNC] AJOUT LBP
...
' LBP 12/05/2010 : PATCH UNC - Dimensionnement du tableau de stockage des noms de fichiers temporaires
ReDim tsTabFichierTemporaire(iNbrDoc)
...
' [DNTMAIL1-2] On spécifie répertoire des paragraphes d'après le canal
'sRepCourrier = System.PrivateProfileString(sFicIniApp, sCleEdition, "REP_COURRIER")
'remplacé par
sRepModele = System.PrivateProfileString(sFicIniApp, sCleEdition, "REP_COURRIER")
sRepPara = System.PrivateProfileString(sFicCourrierIni, "COURRIER", "REP_PARA")
sRepCourrier = System.PrivateProfileString(sFicIniApp, sCleEdition, sRepPara)
...
'remplacé par :
sModele = sRepModele + System.PrivateProfileString(sFicCourrierIni, "COURRIER", "MODELE")
...
' On ouvre un nouveau document basé sur le modéle désiré.
    ...
    '       <ITEM>[wrd]Documents.Add</ITEM>
    '       <URL>http://go.microsoft.com/?linkid=9718419 </URL>
    '   </CHANGE>
    '</VBA_INSPECTOR>
...
' LBP 12/05/2010 : PATCH UNC - Recopie en local du fichier d'entête car la fusion ne supporte pas les chemins UNC
    bRet = bCopyTemporaireFichierEntete(sFicCourrierIni, sSuffixe, tsTabFichierTemporaire(iCpt))
    If Not bRet Then GoTo GestionErreur
' FIN LBP 12/05/2010 : PATCH UNC

    ' On va ensuite le fusionner et le sauvegarder dans lr répertoire prévu à cet effet..

    'LBP : PATCH UNC - Passe le nom du fichier temporaire d'entête en parametre
    'iRet = uf_CommencerFusion(sSuffixe, sFicCourrierIni, sRepGenDoc, DocPrincipal)
    iRet = uf_CommencerFusionNew(sSuffixe, sFicCourrierIni, sRepGenDoc, DocPrincipal, tsTabFichierTemporaire(iCpt))
    If iRet < 1 Then
...
' JFF DCMP04020 le 27/04/04 // On place la gestion Count
' On ferme tous les documents actuellement ouverts sans poser de questions.
    If Application.Documents.Count >= 1 Then Documents.Close SaveChanges:=wdDoNotSaveChanges
...
'[SUPPORT_MFP].SIMPA2 Desactivation de la Mise sous pli ancienne méthode.
...
'DBT LBP le 11/05/2010 : PATCH Chemins UNC - Supprime le fichier temp créé
SupprTousFichiersEnteteTemporaire tsTabFichierTemporaire, iNbrDoc
'FIN LBP le 11/05/2010 : PATCH Chemins UNC


' JFF DCMP04020 le 27/04/04 // On met en remarque
'If Application.WindowState <> wdWindowStateMinimize Then Application.WindowState = wdWindowStateMinimize
