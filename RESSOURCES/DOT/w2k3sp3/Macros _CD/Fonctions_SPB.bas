Option Explicit
'----------------------------------------------
' Fonction uf_GetVarValues
'
' Description : Récupère les valeurs des variables passées au document
'
' Arguments :   asFicCourrierIni As String
'               asNomDoc As String
'               asVariables() As String
'               asValeurs() As String
'
' Date de création & auteur : 28/09/2011 - FPI - [PM73-2]
'----------------------------------------------
Public Function uf_GetVarValues(asFicCourrierIni As String, asNomDoc As String, _
    ByRef asVariables() As String, ByRef asValeurs() As String)
    
    Dim sStringEntete, sStringDonnee As String
    Dim sDonnee, sEntete As String
    Dim sSection As String
    
    If Dir(asFicCourrierIni) = "" Then GoTo GestionErreur
    
    'Récupération des noms de fichiers entête et données
    sSection = System.PrivateProfileString(asFicCourrierIni, "COURRIER", asNomDoc)
    sDonnee = System.PrivateProfileString(asFicCourrierIni, sSection, "DONNEE")
    sEntete = System.PrivateProfileString(asFicCourrierIni, sSection, "ENTETE")

    If Dir(sDonnee) = "" Or sDonnee = "" Then GoTo GestionErreur
    If Dir(sEntete) = "" Or sEntete = "" Then GoTo GestionErreur
    
    'Lecture des 2 fichiers
    Open sEntete For Input As #1
    Line Input #1, sStringEntete
    Close #1

    Open sDonnee For Input As #1
    Line Input #1, sStringDonnee
    Close #1

    'Parsing des 2 listes
    asVariables = Split(sStringEntete, Chr(9), , vbTextCompare)
    asValeurs = Split(sStringDonnee, Chr(9), , vbTextCompare)
...
'----------------------------------------------
' Fonction uf_RepeterHeader
'
' Description : applique l'entête de la première page sur toute les pages
'               et supprime la dernière page si elle est vierge
'
' Arguments :   aDocPrincipal As Document
'               asFicCourrierIni As String
'               asNomDoc As String
'
' Date de création & auteur : 28/09/2011 - FPI - [PM73-2]
'----------------------------------------------
Public Function uf_RepeterHeader(aDocPrincipal, ByVal asFicCourrierIni As String, ByVal asNomDoc As String)

    Dim i, iTotPage As Integer
    Dim sLignes() As String
    Dim bSuppr As Boolean
    Dim shp As Shape
...
On Error GoTo GestionErreur
    ...
    'Recopier le dernier entête sur toutes les pages
    With aDocPrincipal.ActiveWindow
        
        'Positionnement sur la der page
        .Selection.GoTo wdGoToPage, wdGoToNext, iTotPage
        
        'Mode entête et pied de page
        ...
        .ActivePane.View.SeekView = wdSeekCurrentPageFooter 'wdSeekCurrentPageHeader
        
        'Récupération des valeurs des variables
        uf_GetVarValues asFicCourrierIni, asNomDoc, sVariables, sValeurs
        
        'Remplacement des valeurs dans les zones de texte de l'entête
        For Each shp In .Selection.HeaderFooter.Shapes
        
            sTexte = shp.TextFrame.TextRange.Text
            
            If InStr(1, sTexte, "<", vbTextCompare) > 0 Then
                For i = 1 To UBound(sVariables)
                 shp.TextFrame.TextRange.Find.Execute FindText:="<" & sVariables(i) & ">", _
                    ReplaceWith:=sValeurs(i), Replace:=wdReplaceAll
                Next
                
                shp.TextFrame.AutoSize = False
                shp.Top = CentimetersToPoints(20.26)
                shp.Height = CentimetersToPoints(8.8)
                'shp.TextFrame.MarginTop = shp.Top - shp.Height
        ...
        'Positionnement sur le premier header
        'FPI 25/01/2011 - Ajout du on error à cause des sauts de sections
        On Error Resume Next

        For i = 2 To aDocPrincipal.Sections.Count
              .ActivePane.View.PreviousHeaderFooter
        Next
    ...
    aDocPrincipal.Content.Select
    aDocPrincipal.Fields.Update

   'Répéter l'entête sur toutes les pages
    For i = 1 To aDocPrincipal.Sections.Count
      With aDocPrincipal.Sections(i)
          '.Headers(wdHeaderFooterPrimary).Range.WholeStory
          '.Headers(wdHeaderFooterPrimary).LinkToPrevious = True
          .Footers(wdHeaderFooterPrimary).Range.WholeStory
          .Footers(wdHeaderFooterPrimary).LinkToPrevious = True
      End With
    Next
    
    ' Sélection du contenu de la dernière page
    With aDocPrincipal.ActiveWindow.Selection
        .GoTo wdGoToPage, wdGoToNext, iTotPage
        .Find.ClearFormatting
      
        .EndKey Unit:=wdStory, Extend:=wdExtend
    End With
    
    'Contrôle si le contenu avec que des espaces & char(13)
    sLignes = Split(aDocPrincipal.ActiveWindow.Selection.Text, Chr(13))
    
    bSuppr = True
    
    For i = 1 To UBound(sLignes)
        If Len(Trim(sLignes(i))) <> 0 Then
            bSuppr = False
            Exit For
        End If
    Next
    ...
    If bSuppr Then
        ...
        'Mode d'affichage Normal pour ne pas perdre la mise en page
        If aDocPrincipal.ActiveWindow.View.SplitSpecial = wdPaneNone Then
            aDocPrincipal.ActiveWindow.ActivePane.View.Type = wdNormalView
        Else
            aDocPrincipal.ActiveWindow.View.Type = wdNormalView
        End If
            
        With aDocPrincipal.ActiveWindow.Selection
            'Positionnement en dernière page
            .GoTo wdGoToPage, wdGoToNext, iTotPage
            .Find.ClearFormatting
            
            'Sélection du contenu de la der page
            .EndKey Unit:=wdStory, Extend:=wdExtend
...
uf_RepeterHeader aDocPrincipal, asFicCourrierIni, "DOCUMENT" + asSuffixe ' FPI - 28/09/2011 - [PM73-2]
