...
--------------------------------------------------------------------
--
-- Procédure            :       PS_S_FLUX_CHAUD_KRYS_CDL
-- Auteur               :       JFF
-- Date                 :       08/06/2022
-- Libellé              :        
-- Commentaires         :     -- [20251103172936680][JFF][MIG385]  
...
-- JFF      19/05/2025   [MIGR_SQL2022]
-------------------------------------------------------------------
IF EXISTS ( SELECT * FROM sysobjects WHERE name = 'PS_S_FLUX_CHAUD_KRYS_CDL' AND type = 'P' )
        DROP procedure sysadm.PS_S_FLUX_CHAUD_KRYS_CDL
GO

CREATE procedure sysadm.PS_S_FLUX_CHAUD_KRYS_CDL
	@asCas VarChar ( 50 ),
	@adcIdProd Decimal ( 7 )
As
	...
	Begin
		...
		While @iCpt <= 2
			Begin
				...
				SET @sCommande = 'sqlcmd.exe /S' + @sNomServeur + ' /E /Q"' + 
							'sysadm.PS_S_FLUX_CHAUD_KRYS_CDL ''DATA'', ' + Convert ( VarChar ( 7 ), @dcIdProd ) +  
							'" /o' + @sFicOut + ' -w5000 -W ' + @sOsqlOption
