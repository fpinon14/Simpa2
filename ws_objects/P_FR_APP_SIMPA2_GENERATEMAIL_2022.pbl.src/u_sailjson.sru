HA$PBExportHeader$u_sailjson.sru
forward
global type u_sailjson from nonvisualobject
end type
type json_pair from structure within u_sailjson
end type
end forward
...
global u_sailjson u_sailjson

type variables
/*
	u_sailjson:from www.pblsoft.com
...
private function integer of_readobject (u_sailjson vjson, integer alevel)
private subroutine of_error (string msg)
private subroutine skipspc ()
private function long findpairindex (string as_name)
private function string of_getformat (u_sailjson ajson, string ident)
...
private subroutine of_buildtree (u_sailjson ajson, long handle)
...
public function string getstringattribute (string itemname)
public function decimal getdecimalattribute (string itemname)
public function long getlongattribute (string itemname)
end prototypes
...
//"u_sailjson.getattribute: attribute "+string(itemname,"[general]")+" does not exists!")
end subroutine
...
private function string of_getformat (u_sailjson ajson, string ident);string ls, ls1, ls_return, ls_ident, ls_rtn
...
for li = 1 to li_max
	...
	if classname(la) = 'u_sailjson' then
		ls = ident + ls + '{'+ls_rtn
...
private subroutine of_buildtree (u_sailjson ajson, long handle);string ls
...
public function any addarrayitem (string arrayname);u_sailjson ljson
any jsons[]
json_pair pair
integer index
...
public function any addobject (string objname);u_sailjson ljson
json_pair pair
integer index
...
public function string getstringattribute (string itemname);Any aResult
String sResult

SetNull(sResult)

aResult=getattribute(itemname)

if not isnull(aResult) Then sResult=string(aResult)

Return sresult
end function

public function decimal getdecimalattribute (string itemname);Any aResult
decimal dcResult
...
if not isnull(aResult) Then 
	//if isnumber(aResult) Then	dcResult=aResult
	if classname(aResult)="decimal" Then	dcResult=aResult
End if

Return dcResult
end function

public function long getlongattribute (string itemname);Any aResult
long lResult
decimal dcResult
...
if not isnull(aResult) Then 
	if isnumber(aResult) Then	
		dcResult=aResult
		lResult=Long(string(dcResult))
	end if
End if
...
on u_sailjson.create
call super::create
TriggerEvent( this, "constructor" )
end on

on u_sailjson.destroy
